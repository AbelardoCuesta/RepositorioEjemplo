...
export class AlbumCompartirComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.albumId = parseInt(this.router.snapshot.params.userId)
  ...
  compartirAlbum(compartirAlbum: Album){
    ...
    this.albumService.compartirAlbum(this.usuariosAgregar, this.router.snapshot.params.albumId)
    .subscribe(album => {
  ...
  showSuccess(album: Album) {
    this.toastr.success(`El album fue compartido exitosamente!`, "Compartir exitoso");
  }
