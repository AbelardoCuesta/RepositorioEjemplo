...
export class CancionDetailComponent implements OnInit {
  ...
  goToCompartir(){
    this.routerPath.navigate([`/canciones/compartir/${this.cancion.id}/${this.userId}/${this.token}`])
  }

  crearComentario(){
    this.routerPath.navigate([`/canciones/comentario/${this.cancion.id}/${this.userId}/${this.token}`])
  }
