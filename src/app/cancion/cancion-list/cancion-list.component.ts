...
export class CancionListComponent implements OnInit {
  ...
  onSelect(cancion: Cancion, indice: number){
    ...
    this.tipoComentario=TipoComentario.Cancion;
    if(cancion){
      ...
      this.cancionService.getComentariosCancion(cancion.id,this.tipoComentario).subscribe(comentarios=>{
        this.cancionSeleccionada.comentarios=comentarios
      })
